...
//    if (result) console.log('link_mo', curr_link, orig_link); //T
    ...
    var node_ib  = function(curr_node) {
      var curr_name   = curr_node.node.species;
      var orig_name   = orig_link.source.node.species;
    ...
    var node_im  = function(curr_node) {
      var curr_name   = curr_node.node.species;
      var orig_name   = orig_link.target.node.species;
    ...
    var src_name  = orig_link.source.node.species;
    var tgt_name  = orig_link.target.node.species;
  ...
  //   the sidebar to show consistent colors and text
    ...
    var link_ib  = function(curr_link) {
      var curr_name   = curr_link.target.node.species;
      var orig_name   = orig_node.node.species;
    ...
    var link_im  = function(curr_link) {
      var curr_name   = curr_link.source.node.species;
      var orig_name   = orig_node.node.species;
    ...
    var src_tmp   = g.sources[orig_node.node.species];
    var sources   = src_tmp ? Object.keys(src_tmp).sort().join('<br>') : '';

    var targets   = orig_node.node.inputSpecies.sort().join('<br>');

    var html_ib   = on_mouseover_h('ib', sources);
    var html_im   = on_mouseover_h('im', targets);
    var html      = '<h3 class="mo">Node</h3>'
                  + '<span class="mo">' + orig_node.node.species + '</span>'
                  + html_ib + html_im;

    notes.html(html);

    precis.text(orig_node.node.species);
  }
};
